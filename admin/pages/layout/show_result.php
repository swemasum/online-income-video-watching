            ...
            <thead>
              ...
              <?php
                include_once "../../../connection.php";
                    ...
                    if(isset($_GET['exam_no'])){
$exam_no=$_GET['exam_no'];
                    }


                     ///generate quary
                   $sql2 = "SELECT batch_no FROM add_batch";
                   $result2 = mysqli_query($conn, $sql2);
$final_add='';
                   if (mysqli_num_rows($result2) > 0) {
                       // output data of each row
                       while($row2 = mysqli_fetch_assoc($result2)) {
                           $sql2_add= "result.batch_no='".$row2['batch_no']."'";
                           $final_add=$final_add." or ".$sql2_add;
                       }
                   } else {

                   }
                  $final_add = substr($final_add,3);
...
///generate quary end
if(isset($_GET['exam_no'])){
  $sql = "SELECT current_student.roll,result.current_student_id,current_student.s_name,current_student.college,result.cre,result.mcq,(result.cre+result.mcq) as total FROM result left join current_student on result.current_student_id=current_student.current_student_id where ($final_add) and exam_no='$exam_no' order by total desc";

}else{
  $sql = "SELECT current_student.roll,result.current_student_id,current_student.s_name,current_student.college,sum(result.cre) as cre,sum(result.mcq) as mcq,(sum(result.cre)+sum(result.mcq)) as total FROM result left join current_student on result.current_student_id=current_student.current_student_id where ($final_add) group by current_student_id order by total desc";
