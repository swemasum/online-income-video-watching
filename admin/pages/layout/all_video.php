                 ...
                 <tbody>
                   <?php
                     include_once "../../../connection.php";
                         ...
                         if(isset($_POST['date'])){
                             $date=$_POST['date'];
                               $sql = "SELECT * FROM video where date='$date'";
                         }else if(isset($_POST['search'])){
                                 $search=$_POST['search'];
                                 $sql = "SELECT * FROM video where title like '%$search%' or description like '%$search%'";
                         ...
                         if (mysqli_num_rows($result) > 0) {
                           ...
                           while($row = mysqli_fetch_assoc($result)) {
                                ...
                                $link=$row['link'];
                    $pieces =explode("src=", $link);

                  $after_src=$pieces[1];
                   $pieces2 =explode(" ", $after_src);
                    $url=$pieces2[0];
                    $str_to_insert="?autoplay=1";
                    $pos=strlen($url)-1;

                              $newstr = substr_replace($url, $str_to_insert, $pos, 0);



                             echo "<tr class='info' >";
                               echo "<td> Date: " . $row["date"]."<br>Title: ".$row['title']."<br>Description: ".$row["description"]."</td>";

                               echo '<td><iframe width="300" height="215"  src='.$url.'frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></td>';


                               echo '<td> <img  height="100px" src="../../../video/'.$row["image"].'"</td>';
                               echo '<td><a onclick="return check();" href="delete_video.php?id='.$row['v_id']. '"class="btn btn-info btn-sm">
               <span class="glyphicon glyphicon-remove"></span>
             </a>
             <a href="edit_video.php?id='.$row['v_id']. '"class="btn btn-info btn-sm">
