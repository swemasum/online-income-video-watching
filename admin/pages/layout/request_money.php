      ...
      <div class="row">
       <div class="col-md-12">
           <div class="card">
               ...
               <form class="" action="all_user.php" method="post">

             <div class="col-md-4 col-sm-4">
                 <div class="form-group ">
              <input type="text" name="search" class="form-control" value="" PLACEHOLDER="Name/Phone/email">
                     </div>
             </div>
...
$sql_request = "SELECT * FROM request order by date desc";
...
if (mysqli_num_rows($result_request) > 0) {
  ...
  while($row_request = mysqli_fetch_assoc($result_request)) {
    $user_id=$row_request['user_id'];
$r_id=$row_request['r_id'];


                         if(isset($_POST['search'])){
                                $search=$_POST['search'];
                                $sql = "SELECT * FROM user where (name like '%$search%' or phone_no like '%$search%'or bd_no like '%$search%' or email like '%$search%' date like '%$search%') and status='active' and and u_id='$user_id'";

                            }else{
                                $sql = "SELECT * FROM user where status='active' and u_id='$user_id'";
                            }
                        ...
                        if (mysqli_num_rows($result) > 0) {
                          ...
                          while($row = mysqli_fetch_assoc($result)) {
                            echo "<tr class='info'>";
                            echo "<td>" . $row["date"]."</td>";
                              echo "<td>Id: " . $row["u_id"]."<br>".$row["name"]."<br>Address".$row["address"]."<br>Phone:".$row["phone_no"]."</td>";
                              echo "<td>email:" . $row["email"]."<br>Password:".$row["password"]."</td>";
                              echo "<td>" . $row["status"]."<br>Date:".$row["date"]."</td>";
                              echo "<td>number: " . $row["bd_no"]."<br>Type: ".$row['payment_type']."</td>";

                              echo '<td><a href="payment_status.php?id='.$row['u_id']. '&ref_num='.$row['ref_num'].'"class="btn btn-info btn-lg">
              <span class="glyphicon glyphicon-usd"></span>
            </a>

    <a onclick="return check();" href="delete_request_money.php?id='.$r_id. '"class="btn btn-info btn-lg">
